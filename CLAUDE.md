...
Most of what sits below the UI is still simulated: five of the eight games are decorative (no game engine — the player animates a CRT scene and increments the score on a timer), and leaderboard rows are produced by a deterministic LCG in `lib/scores.ts` and never persisted. Treat those as deliberate boundaries; a spec decides when one of them changes.

**Three games are the exception — they are real.** Each one is a pure rules module in `lib/` (no `document`, no `window`, no canvas) plus a client component in `components/` that owns the canvas, the `requestAnimationFrame` loop, the keyboard and the on-screen pad.
...
- **`ARKANOID`** (SPEC 15, `lib/arkanoid.ts` + `components/arkanoid-game.tsx`) is the eighth catalog entry — added, not a replacement, so every existing id and every existing leaderboard is untouched — and ports `references/started-games/04-arkanoid/`: 800 × 600 board (exactly the `4 / 3` of `.crt-screen`, so it needs no aspect modifier), 10 × 6 wall of 64 × 24 bricks, three lives, `10 × level` per brick and `100 × level` per cleared wall. Levels are infinite: 1–5 are the reference's five hand-made walls and 6 onward are generated by the same LCG as `lib/scores.ts` — copied, never imported — seeded with the level number, so they are pure, vertically symmetric and never under 8 bricks. Four rules depart from the reference on purpose: the paddle bounce takes its angle from the impact point (capped at 60°) instead of only flipping `vy`, so the player can aim; ball speed is a two-slope curve from 224 px/s capped at 800; `step()` splits the advance into 8 px substeps and clamps `dt` to 50 ms, so nothing is ever tunneled through; and there is no win state, because the levels do not end.

`components/game-player.tsx` decides which one mounts through the `ENGINES` registry — `{ Component, lives, screen }` keyed by `game.id`, where `screen` is the modifier added to `.crt-screen` and is empty when the engine already fits the tube's `4 / 3` (as `ARKANOID` does). Adding a fourth engine means adding a row there, not another branch.

Two rules hold that boundary. **The HUD is common to all eight games** — player, score, hearts, level, and the `PAUSA` / `FIN` / `SALIR` buttons — so nothing game-specific goes into it and no pause control goes inside the screen; an engine only reports its numbers through `onRun`. What a game may draw inside its own canvas is its own state: `ASTEROIDES` paints the countdown of the active drop, never the score, the lives or the level, and `ARKANOID` paints nothing but the board. And **the score is still not persisted**: the engine computes a real one, but `GUARDAR PUNTUACIÓN` in the modal stays decorative. The modal shows the session's name — no editable input — and a guest gets an invitation to sign in instead of the save button: it sends the run to `/auth` and back through the URL (`/jugar/tetrix?puntuacion=&nivel=`), which `app/jugar/[id]/page.tsx` validates and hands to the player as `restored`. Nothing is written anywhere; real persistence needs a table, RLS and a migration, so it is a spec of its own.
...
Twelve components in `components/`. Ten are client components (`"use client"`) because they hold state or DOM handlers — `nav.tsx`, `session-provider.tsx`, `library-browser.tsx`, `game-card.tsx`, `game-player.tsx`, `tetris-game.tsx`, `asteroids-game.tsx`, `arkanoid-game.tsx`, `auth-form.tsx`, `hall-of-fame.tsx`. Two are server components and should stay that way: `footer.tsx` and `leaderboard.tsx`, which renders rows it receives as props.

Two mock-data modules in `lib/` — plus `tetris.ts`, `asteroids.ts` and `arkanoid.ts`, which are not mock data but the rules of the three real games (see above):

- `games.ts` — `Game`, `GameColor`, `GameCat`, `CatFilter`, the `GAMES` array, `CATS`, and `getGame(id)`. A game may carry an optional `image` (a real screenshot under `public/juegos/`) that replaces the CSS-drawn cover; only `TETRIX`, `ASTEROIDES` and `ARKANOID` have one, because only they exist to be photographed.
...
`tests/screens.spec.ts` holds the whole suite, grouped by `describe` blocks: reference screenshots, library, detail, player, `tetrix`, `asteroides`, `arkanoid`, auth, hall of fame, and responsive. The three engine blocks do not freeze the clock — their loops need a live `requestAnimationFrame` — and none asserts anything that depends on the randomness of a run.
