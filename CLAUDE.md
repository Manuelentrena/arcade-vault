...
Most of what sits below the UI is still simulated: five of the seven games are decorative (no game engine — the player animates a CRT scene and increments the score on a timer), and leaderboard rows are produced by a deterministic LCG in `lib/scores.ts` and never persisted. Treat those as deliberate boundaries; a spec decides when one of them changes.

**Two games are the exception — they are real.** Each one is a pure rules module in `lib/` (no `document`, no `window`, no canvas) plus a client component in `components/` that owns the canvas, the `requestAnimationFrame` loop, the keyboard and the on-screen pad.

- **`TETRIX`** (SPEC 13, `lib/tetris.ts` + `components/tetris-game.tsx`) replaced `BLOQUE BUSTER` and `CAÍDA` with a single playable entry, ported from `references/started-games/03-tetris/`: 10 × 20 board, the seven standard tetrominoes, wall kicks `[0, ±1, ±2]`, ghost piece, `NEXT` preview, soft and hard drop, `min(10, floor(lines / 10) + 1)` for the level and `max(100, 1000 − (level − 1) × 90)` ms for the fall — so the level caps at 10 and the interval at 190 ms. One life: the top-out ends the run.
- **`ASTEROIDES`** (SPEC 14, `lib/asteroids.ts` + `components/asteroids-game.tsx`) replaced the decorative `ROCAS` entry — new id, so `/juego/rocas` and `/jugar/rocas` are 404 — and ported `references/started-games/02-asteroids/`: 800 × 600 world that wraps at the edges, ship with inertia, rocks of three sizes worth 20 / 50 / 100 that split in two on each hit, `min(10, 3 + level)` rocks per wave with the level itself uncapped, and two drops, `3x` and shield, never more than one on the field. Four rules depart from the reference on purpose: drag is per second (`0.987 ** 60` raised to `dt`), not per frame; one life instead of three; `fire` is a held state, not an edge, so the on-screen button is playable; and the score has no level multiplier.

`components/game-player.tsx` decides which one mounts through the `ENGINES` registry — `{ Component, lives, screen }` keyed by `game.id`, where `screen` is the modifier added to `.crt-screen`. Adding a third engine means adding a row there, not another branch.

Two rules hold that boundary. **The HUD is common to all seven games** — player, score, hearts, level, and the `PAUSA` / `FIN` / `SALIR` buttons — so nothing game-specific goes into it and no pause control goes inside the screen; an engine only reports its numbers through `onRun`. What a game may draw inside its own canvas is its own state: `ASTEROIDES` paints the countdown of the active drop, never the score, the lives or the level. And **the score is still not persisted**: the engine computes a real one, but `GUARDAR PUNTUACIÓN` in the modal stays decorative. The modal shows the session's name — no editable input — and a guest gets an invitation to sign in instead of the save button: it sends the run to `/auth` and back through the URL (`/jugar/tetrix?puntuacion=&nivel=`), which `app/jugar/[id]/page.tsx` validates and hands to the player as `restored`. Nothing is written anywhere; real persistence needs a table, RLS and a migration, so it is a spec of its own.
...
Eleven components in `components/`. Nine are client components (`"use client"`) because they hold state or DOM handlers — `nav.tsx`, `session-provider.tsx`, `library-browser.tsx`, `game-card.tsx`, `game-player.tsx`, `tetris-game.tsx`, `asteroids-game.tsx`, `auth-form.tsx`, `hall-of-fame.tsx`. Two are server components and should stay that way: `footer.tsx` and `leaderboard.tsx`, which renders rows it receives as props.

Two mock-data modules in `lib/` — plus `tetris.ts` and `asteroids.ts`, which are not mock data but the rules of the two real games (see above):

- `games.ts` — `Game`, `GameColor`, `GameCat`, `CatFilter`, the `GAMES` array, `CATS`, and `getGame(id)`. A game may carry an optional `image` (a real screenshot under `public/juegos/`) that replaces the CSS-drawn cover; only `TETRIX` and `ASTEROIDES` have one, because only they exist to be photographed.
...
`tests/screens.spec.ts` holds the whole suite, grouped by `describe` blocks: reference screenshots, library, detail, player, `tetrix`, `asteroides`, auth, hall of fame, and responsive. The two engine blocks do not freeze the clock — their loops need a live `requestAnimationFrame` — and neither asserts anything that depends on the randomness of a run.
